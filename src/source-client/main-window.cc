    ...
    , ui(new Ui::MainWindow)
{
    ...
    SDL(); /*
         ...
         for (int i = 0; i < in_avfc->nb_streams; i++)
         {
             ...
             //复制配置信息：输入编码器->输出流
             // int re =
         avcodec_copy_context(out_Stream->codec,in_avfc->streams[i]->codec);//codec已经是过时的，但如果是mp4
...
int MainWindow::SDL()
{
    if (SDL_Init(SDL_INIT_VIDEO)) //成功，非0
        std::cout << "SDL初始化失败";
    SDL_Window *screen = SDL_CreateWindow("新SDL窗口", 0, 0, WIDTH, HEIGHT, SDL_WINDOW_RESIZABLE);
    if (screen == NULL)
        std::cout << "SDL窗口创建失败";
