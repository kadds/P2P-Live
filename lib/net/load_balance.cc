...
{
    /// content server join
    inner_server.on_client_join([this](tcp::server_t &server, tcp::connection_t conn) {
        if (server_handler)
...
{
    server.on_client_join([this](tcp::server_t &server, tcp::connection_t conn) {
...
front_server_t &front_server_t::on_inner_server_join(server_join_handler_t handler)
{
