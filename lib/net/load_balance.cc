...
{
    /// content server join
    inner_server.at_client_join([this](tcp::server_t &server, socket_t *client) {
        if (server_handler)
            server_handler(*this, client);
    });
    ...
    server.at_client_join([this](tcp::server_t &server, socket_t *client) {
        ...
        if (handler)
        {
            if (handler(*this, request, respond, client))
            {
                ...
                co::await(socket_awrite, client, respond_buffer);
            }
        }
    });
